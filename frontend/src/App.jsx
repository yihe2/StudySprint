...
function App() {
  ...
  async function handleSnoozeToday() {
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/goals/actions/snooze-today`, {
        method: "PATCH",
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to snooze today's goals.");
      }
      await reloadData();
    } catch (bulkError) {
      setError(bulkError.message);
  ...
  async function handlePinToday() {
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/goals/actions/pin-today`, {
        method: "PATCH",
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to pin today's goals.");
      }
      await reloadData();
    } catch (bulkError) {
      setError(bulkError.message);
  ...
  async function handleActivateOverdue() {
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/goals/actions/activate-overdue`, {
        method: "PATCH",
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to activate overdue goals.");
      }
      await reloadData();
    } catch (bulkError) {
      setError(bulkError.message);
  ...
  async function handlePinOverdue() {
    setError("");
    try {
      const response = await fetch(`${API_BASE}/api/goals/actions/pin-overdue`, {
        method: "PATCH",
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || "Failed to pin overdue goals.");
      }
      await reloadData();
    } catch (bulkError) {
      setError(bulkError.message);
  ...
  return (
    <main className="app">
      ...
      <section className="card">
        ...
        <p className="stats">
          Archive Share: <strong>{stats.archivedRate || 0}%</strong> of listed goals archived
        </p>
        <p className="stats">
        ...
        <p className="stats">
          Planning Risk: <strong>{stats.highPriorityUndated || 0}</strong> high-priority unscheduled | <strong>{stats.staleOverdue || 0}</strong> stale overdue
        </p>
        <p className="stats">
        ...
        <div className="bulk-actions">
          ...
          <button type="button" onClick={handleActivateOverdue}>
            Activate Overdue
          </button>
